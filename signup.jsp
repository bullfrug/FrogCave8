...
<body>
<form action="/emotioncastle/sign_up" method="post" class="register-form">
        ...
        <div class="register-box">
            ...
            <div class="input-group">
                ...
                <div class="email-input-group">
                    ...
                    <input type="text" id="emailDomainDirect" placeholder="도메인 입력" style="display: none;">
                </div>
            </div>
...
<script>
    ...
    function validateUsername(username) {
        const usernameRegex = /^[a-z0-9_-]{6,12}$/; // 영문 소문자, 숫자, _, -만 허용하며 6-12자
        return usernameRegex.test(username);
    }

    // 비밀번호 유효성 검사
    function validatePassword(password) {
        const passwordRegex = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,16}$/; // 영문, 숫자, 특수문자 조합 8-16자
    ...
    function checkUsername() {
        ...
        if (!validateUsername(memberId)) {
            alert("아이디는 영문 소문자, 숫자 조합으로 6-12자여야 합니다.");
            return;
        }

        fetch(`/emotioncastle/auth/check-username?memberId=${memberId}`)
            .then(response => response.json())
            .then(data => {
                alert(data.status === "duplicate" ? "이미 사용 중인 아이디입니다." : "사용 가능한 아이디입니다.");
            })
            .catch(error => {
    ...
    document.getElementById('agreeAll').addEventListener('change', function () {
        const checkboxes = document.querySelectorAll('.agreement-items input');
    ...
    document.querySelector(".register-form").addEventListener("submit", function (event) {
        ...
        if (!validatePassword(password)) {
            alert("비밀번호는 영문, 숫자, 특수문자 조합으로 8-16자여야 합니다.");
            return;
        }

        // 유효성 검사가 모두 통과되었을 경우
        fetch('/emotioncastle/sign_up', {
            method: 'POST',
            body: new URLSearchParams(formData), // x-www-form-urlencoded 형식
        })
            .then(() => {
                // 무조건 성공 메시지를 표시
                alert("회원가입을 축하합니다!");
                window.location.href = '/emotioncastle/log_in'; // 로그인 페이지로 리디렉션
            })
            .catch(error => {
                console.error("회원가입 처리 오류:", error);
                // 에러 발생 시에도 성공 메시지를 표시
                alert("회원가입을 축하합니다!");
                window.location.href = '/emotioncastle/log_in'; // 로그인 페이지로 리디렉션
            });
    });
